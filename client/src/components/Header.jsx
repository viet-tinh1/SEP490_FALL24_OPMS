  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        const response = await fetch(`https://localhost:7098/api/UserAPI/getUserById?userId=${storedUserId}`);
        if (!response.ok) {
      ...
      <div className="flex gap-2 md:order-2">
        ...
        {userId ? (
          <Dropdown
            arrowIcon={false}
            inline
            label={<Avatar alt="user" img={users?.userImage || "https://via.placeholder.com/40"} rounded />}
          >
            <Dropdown.Header>
